...
#[structopt(about = "advent of code 2015")]
enum Subcommand {
...
impl Subcommand {
    fn run(self) -> Result<()> {
        match self {
            ...
            Self::Url { day } => {
                println!("{}", aoc2015::website::url_for_day(day.into()));
                Ok(())
            }
            Self::Init {
                day,
                ...
                aoc2015::day::initialize(&config, day.into(), skip_create_crate, skip_get_input)?;
                Ok(())
            }
        }
    }
...
impl ConfigOpts {
    fn run(self) -> Result<()> {
        match self {
            Self::Path => println!("{}", aoc2015::config::path().display()),
            Self::Show => {
                let data = std::fs::read_to_string(aoc2015::config::path())?;
                println!("{}", data);
            }
